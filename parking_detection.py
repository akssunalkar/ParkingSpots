...
def mask_image(img, mask):
    ...
    INPUTS:
        ...
        img: The image which has to be masked

        mask: The mask that has to overlayed on the image 

    OUTPUTS: 

    The mask overlayed on the image.
...
def get_lines(img, roi, param=[6,20,80,20,35]):
    ...
    imsave("Outputs/cannyed_image.png", cannyed_image)
    if type(roi) == type([]):
...
def get_scatter(lines):
    ...
    np.savetxt("Outputs/foo.csv", X, delimiter=",")
...
def hc(n_clust):
    ...
    X = np.genfromtxt('Outputs/foo.csv',delimiter=',')
    ...
    plt.savefig("Outputs/dendogram.png")
...
def highlight_spots(img, long_lines, car_lines,border):
...
if __name__ == "__main__":
    ...
    if not os.path.isfile("output.npy"):
        city_segment.segmentation(filen, "Outputs/output.jpg")
    ...
    cv2.imwrite("Outputs/woslope.png",line_image)

    gray_image = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)

    # Save images
    cv2.imwrite("Outputs/mask_image.png",gray_image*mask_road)
