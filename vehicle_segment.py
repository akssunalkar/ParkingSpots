...
def segmentation(filename, output_file="output.png"):
    ...
    # Get only the cars
    ids_car = cars_only(ids, ids, scores, 0.9)
    scores_car = cars_only(scores, ids, scores, 0.9)
    bboxes_car = cars_only(bboxes, ids, scores, 0.9)
    masks_car = cars_only(masks, ids, scores, 0.9)
    ...
    cv2.imwrite("Outputs/image_bbox_floor.png", orig_img)
